...
class FiveStageThermocyclingGUI:
    def __init__(self, root):
        ...
        self.cycle_entry.insert(0, "30")  # Default cycle number
        ...
        self.thermo_frame = ttk.LabelFrame(root, text="Five-Stage Thermocycling Control")
        self.thermo_frame.grid(row=1, column=0, padx=2, pady=2, sticky="ew")

        # Threshold input fields
        self.upper_label = ttk.Label(self.thermo_frame, text="Upper Temp Threshold (°C):")
        self.upper_label.grid(row=7, column=0, padx=2, pady=2)
        self.upper_entry = ttk.Entry(self.thermo_frame)
        self.upper_entry.insert(0, "95")  # Default
        self.upper_entry.grid(row=7, column=1, padx=2, pady=2)

        self.lower_label = ttk.Label(self.thermo_frame, text="Lower Temp Threshold (°C):")
        self.lower_label.grid(row=8, column=0, padx=2, pady=2)
        self.lower_entry = ttk.Entry(self.thermo_frame)
        self.lower_entry.insert(0, "60")  # Default
        self.lower_entry.grid(row=8, column=1, padx=2, pady=2)

        self.setpoint_label = ttk.Label(self.thermo_frame, text="Setpoint (°C):")
        self.setpoint_label.grid(row=9, column=0, padx=2, pady=2)
        self.setpoint_entry = ttk.Entry(self.thermo_frame)
        self.setpoint_entry.insert(0, "60")  # Default
        self.setpoint_entry.grid(row=9, column=1, padx=2, pady=2)

        self.send_thresholds_button = ttk.Button(self.thermo_frame, text="Send Thresholds", command=self.send_thresholds)
        self.send_thresholds_button.grid(row=10, column=0, columnspan=1, padx=2, pady=2)
        ...
        # Phase 1
        ttk.Label(self.thermo_frame, text="Initial denaturation Temp (°C):").grid(row=0, column=0, padx=2, pady=2)
        temp1_entry = ttk.Entry(self.thermo_frame)
        temp1_entry.insert(0, "95")  # preset value for Phase 1
        temp1_entry.grid(row=0, column=1, padx=2, pady=2)
        self.temp_entries.append(temp1_entry)

        ttk.Label(self.thermo_frame, text="Hold Time (s):").grid(row=0, column=2, padx=2, pady=2)
        time1_entry = ttk.Entry(self.thermo_frame)
        time1_entry.insert(0, "300")  # preset value for Phase 1 hold
        time1_entry.grid(row=0, column=3, padx=2, pady=2)
        self.time_entries.append(time1_entry)

        # Phase 2
        ttk.Label(self.thermo_frame, text="Denaturation Temp (°C):").grid(row=1, column=0, padx=2, pady=2)
        temp2_entry = ttk.Entry(self.thermo_frame)
        temp2_entry.insert(0, "95")  # preset value for Phase 2
        temp2_entry.grid(row=1, column=1, padx=2, pady=2)
        self.temp_entries.append(temp2_entry)

        ttk.Label(self.thermo_frame, text="Hold Time (s):").grid(row=1, column=2, padx=2, pady=2)
        time2_entry = ttk.Entry(self.thermo_frame)
        time2_entry.insert(0, "15")  # preset value for Phase 2 hold
        time2_entry.grid(row=1, column=3, padx=2, pady=2)
        self.time_entries.append(time2_entry)

        # Phase 3
        ttk.Label(self.thermo_frame, text="Annealing Temp (°C):").grid(row=2, column=0, padx=2, pady=2)
        temp3_entry = ttk.Entry(self.thermo_frame)
        temp3_entry.insert(0, "60")  # preset value for Phase 3
        temp3_entry.grid(row=2, column=1, padx=2, pady=2)
        self.temp_entries.append(temp3_entry)

        ttk.Label(self.thermo_frame, text="Hold Time (s):").grid(row=2, column=2, padx=2, pady=2)
        time3_entry = ttk.Entry(self.thermo_frame)
        time3_entry.insert(0, "30")  # preset value for Phase 3 hold
        time3_entry.grid(row=2, column=3, padx=2, pady=2)
        self.time_entries.append(time3_entry)

        # Phase 4
        ttk.Label(self.thermo_frame, text="Extension Temp (°C):").grid(row=3, column=0, padx=2, pady=2)
        temp4_entry = ttk.Entry(self.thermo_frame)
        temp4_entry.insert(0, "72")  # preset value for Phase 4
        temp4_entry.grid(row=3, column=1, padx=2, pady=2)
        self.temp_entries.append(temp4_entry)

        ttk.Label(self.thermo_frame, text="Hold Time (s):").grid(row=3, column=2, padx=2, pady=2)
        time4_entry = ttk.Entry(self.thermo_frame)
        time4_entry.insert(0, "30")  # preset value for Phase 4 hold
        time4_entry.grid(row=3, column=3, padx=2, pady=2)
        self.time_entries.append(time4_entry)

        # Phase 5
        ttk.Label(self.thermo_frame, text="Final Extension Temp (°C):").grid(row=4, column=0, padx=2, pady=2)
        temp5_entry = ttk.Entry(self.thermo_frame)
        temp5_entry.insert(0, "72")  # preset value for Phase 5
        temp5_entry.grid(row=4, column=1, padx=2, pady=2)
        self.temp_entries.append(temp5_entry)

        ttk.Label(self.thermo_frame, text="Hold Time (s):").grid(row=4, column=2, padx=2, pady=2)
        time5_entry = ttk.Entry(self.thermo_frame)
        time5_entry.insert(0, "300")  # preset value for Phase 5 hold
        time5_entry.grid(row=4, column=3, padx=2, pady=2)
        self.time_entries.append(time5_entry)
        ...
        self.start_button = ttk.Button(self.thermo_frame, text="Start 5-Stage", command=self.start_5stage_thermocycling)
        self.start_button.grid(row=6, column=0, padx=2, pady=2)

        # Start simple thermocycling button
        self.start_simple_button = ttk.Button(self.thermo_frame, text="Start Ultrafast", command=self.start_simple_thermocycling)
        self.start_simple_button.grid(row=10, column=1, padx=2, pady=2)
        ...
        self.stop_button = ttk.Button(self.thermo_frame, text="Stop", command=self.stop_thermocycling)
        self.stop_button.grid(row=10, column=2, columnspan=2, padx=2, pady=2)
        ...
        self.output_text = tk.Text(self.output_frame, width=70, height=5, font=("Helvetica", 12))  # Set font size to 12
        ...
        self.canvas.get_tk_widget().place(x=715, y=80, width=550, height=450)  # Use place to position the widget
    ...
    def send_thresholds(self):
        if self.connected:
            upper_temp = self.upper_entry.get().strip()
            lower_temp = self.lower_entry.get().strip()
            setpt = self.setpoint_entry.get().strip()

            try:
                upper = float(upper_temp)
                lower = float(lower_temp)
                setpoint = float(setpt)
                command = f"T{upper},{lower},{setpoint}\n"
                self.ser.write(command.encode())
                self.ser.flush()
                self.status_label.config(text=f"Sent thresholds: Upper={upper}, Lower={lower}, Setpoint={setpoint}")
            except ValueError:
    ...
    def start_5stage_thermocycling(self):
        if self.connected:
            self.ser.write(b"START\n")  # Same as before
            self.status_label.config(text="Started 5-Stage Thermocycling")
            self.ser.flush()
            threading.Thread(target=self.read_serial_output, daemon=True).start()

    def start_simple_thermocycling(self):
        if self.connected:
            self.ser.write(b"START_SIMPLE\n")  # Send "START_SIMPLE" to Arduino
            self.status_label.config(text="Started Simple Thermocycling")
            self.ser.flush()
            threading.Thread(target=self.read_serial_output, daemon=True).start()
    ...
    def update_plot(self, frame):
        ...
        self.ax.plot(smoothed_x_data, smoothed_y_data, linestyle='-')  # Connect points with lines
...
def main():
    root = tk.Tk()
    root.geometry("1300x650")  # Set the window size to 1000 width and 600 height
